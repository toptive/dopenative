...
export const TranslationContext = /*#__PURE__*/React.createContext({});
export const TranslationProvider = _ref => {
  let {
    children,
    translations
  } = _ref;
  ...
  i18n.fallbacks = true; // update layout direction

  I18nManager.forceRTL(Localization.isRTL);
  const localized = useCallback((key, config) => i18n.t(key, { ...config,
    locale
  }).includes('missing') ? key : i18n.t(key, { ...config,
    locale
  }), [locale]);
